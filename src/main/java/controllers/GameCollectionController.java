...
public class GameCollectionController extends BaseController {
     ...
     */
    private void loadUserCollection(List<game> userCollection) {
        if (loggedInUserID <= 0) {
     ...
     */
    private boolean sortFilteredCollection() {
        if (filteredGameCollection == null || filteredGameCollection.isEmpty()) {
     ...
     */
    private void searchCollection(String searchedGame) {
    ...
    private Node createGameBox(game gameItem) {
        ...
        try {
            ...
            System.err.println("⚠️ Error loading image: " + imageUrl);
        }
     ...
     */
    @FXML
    private void clickAddGame(javafx.event.ActionEvent event) {
        try {
