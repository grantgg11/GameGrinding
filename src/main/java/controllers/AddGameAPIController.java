...
public class AddGameAPIController extends BaseController {
    @FXML TextField searchField;                        // Search input field for game titles
    @FXML private Button searchButton;                          // Button to trigger API search
    @FXML FlowPane resultsFlowPane;                     // Container for displaying search result cards
    @FXML ScrollPane scrollPane;                        // ScrollPane wrapping the results view
    @FXML private CheckBox checkBox;                            // Checkbox to select individual games (used in tile UI)
    @FXML Button addButton;                             // Button to add selected games to user's collection
    @FXML StackPane loadingOverlay;                     // Overlay to indicate loading state
     ...
     */
    @FXML void handleSearch() throws ParseException, InterruptedException, ExecutionException {
            ...
            try {
                ...
                new Thread(() -> {
                    ...
                    Platform.runLater(() -> {
                        ...
                        alert.showError(
                            "API Unavailable",
                            "Unable to connect to the game database.",
     ...
     */
    protected Image loadImageWithFallback(String url) {
        try {
     ...
     */
    @FXML protected void handleAddSelectedGames() throws ParseException, InterruptedException, ExecutionException {
