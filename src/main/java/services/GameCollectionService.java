...
public class GameCollectionService {
    ...
    public List<game> getUserCollection(int userID) { 
        ...
        return collectionDAO.getAllGamesInCollection(userID);
    }
     ...
     * @param userID The user ID.
     */
    public void removeGameFromCollection(int gameID, int userID) {
		if (userID <= 0) {
			System.out.println("Invalid user ID.");
			return;
		}
		collectionDAO.removeGameFromCollection(gameID, userID);
	}
